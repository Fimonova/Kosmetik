...
RESET_EMAIL = (os.getenv("RESET_EMAIL") or "").strip().lower()
RESET_CODE  = (os.getenv("RESET_CODE") or "").strip()
...
class ForgotSimpleStart(BaseModel):
    email: str
    app_id: str | None = DEFAULT_APP_ID

class ForgotSimpleFinish(BaseModel):
    email: str
    code: str
    new_password: str
    app_id: str | None = DEFAULT_APP_ID
...
@app.post("/forgot_simple_start")
async def forgot_simple_start(
    payload: ForgotSimpleStart,
    x_signature: str = Header(None),
    x_timestamp: str = Header(None),
    authorization: str = Header(None),
):
    ...
    if not RESET_EMAIL:
        raise HTTPException(500, "RESET_EMAIL is not configured")
...
@app.post("/forgot_simple_finish")
async def forgot_simple_finish(
    payload: ForgotSimpleFinish,
    x_signature: str = Header(None),
    x_timestamp: str = Header(None),
    authorization: str = Header(None),
):
    ...
    if not RESET_EMAIL or not RESET_CODE:
        raise HTTPException(500, "RESET_EMAIL/RESET_CODE not configured")
    ...
    app_id = payload.app_id or DEFAULT_APP_ID

    # تغيّر كلمة السر + تصفير القفل
